...
export default function AdminDashboard() {
  ...
  const [shippingInput, setShippingInput] = useState('120');
  ...
  useEffect(() => {
    ...
    ]).then(([orders, products, categories, settings]) => {
      setStats({
        orders: orders.length,
        pending: orders.filter(o => o.status === 'pending').length,
        products: products.length,
        categories: categories.length,
        revenue: orders.filter(o => o.status === 'delivered' || o.status === 'completed').reduce((sum, o) => sum + ((o.total || 0) - (o.shipping_fee || 0)), 0),
      });
      if (settings.shipping_fee) {
        setShippingFee(parseFloat(settings.shipping_fee));
        setShippingInput(settings.shipping_fee);
      }
    });
  }, []);
  ...
  const updateShipping = async () => {
    ...
    await api.updateSetting('shipping_fee', String(val));
    setShippingFee(val);
    setSaved(true);
  ...
  return (
    <div>
      ...
      <div className="border border-gray-100 rounded-2xl p-6">
        <h3 className="font-serif text-lg font-medium mb-4">Delivery Fee Settings</h3>
        <div className="flex items-center gap-3">
          ...
          />
          <button onClick={updateShipping} className="btn-primary text-sm !py-2.5">
            {saved ? 'Saved!' : 'Update'}
          </button>
          <span className="text-xs text-gray-400">Current: ৳{shippingFee}</span>
        </div>
      </div>
    </div>
  );
